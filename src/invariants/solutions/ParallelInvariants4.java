...
public class ParallelInvariants4 {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        final AtomicReference<Circle> circle = new AtomicReference<>(new Circle(1.));

        Runnable r = () -> {
            for (long i = 1; i < 10_000_000; ++i) {
                circle.set(circle.get().updateCircle(i));
            }
        };

        Runnable c = new Thread(() -> {
            while (!stop) {
                Circle currentCircle = circle.get();

                checkCircleConsistency(currentCircle, inconsistentObservationCounter);
            }
        });
